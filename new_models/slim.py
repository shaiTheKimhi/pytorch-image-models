...
class SlimNetEx(nn.Module):
    ...
    def fit_channels(self, layers, skip): #fix channels reduced by constant
        ...
        layers[4][0].block[0][0] = nn.Conv2d(in_channels= 32 + skip * 8 + start, out_channels=240, kernel_size=(1,1), bias=False)  #40 to 32
        ...
        layers[5][0].block[0][0] = nn.Conv2d(in_channels= 44 + skip * 8 + start, out_channels=480, kernel_size=(1,1),bias=False) #80 to 44
        ...
        layers[6][0].block[0][0] = nn.Conv2d(in_channels= 62 + skip * 8 + start , out_channels=672, kernel_size=(1,1), bias=False) #112 to 62
        ...
        layers[7][0].block[0][0] = nn.Conv2d(in_channels= 96 + skip * 8 + start, out_channels=1152, kernel_size=(1,1), bias=False) #196 to 96
        ...
        layers[8][0] = nn.Conv2d(in_channels= 196 + skip * 8 + start, out_channels=1280, kernel_size=(1,1), bias=False) #320 to 196
